 ...
 */
export function loadPreset(profileName?: string): Record<string, string | number | boolean> {
  const profile = profileName?.toUpperCase() || process.env.CONFIG_PROFILE?.toUpperCase() || "MEDIUM";
  const preset = PRESETS[profile];

  if (!preset) {
    console.warn(
      `[CONFIG] Unknown profile: ${profile}. Available: ${Object.keys(PRESETS).join(", ")}. Using MEDIUM.`
    );
    return PRESETS.MEDIUM.settings;
  }
...
export function mergePreset(
  ...
  const preset = PRESETS[profileName.toUpperCase()];
  if (!preset) throw new Error(`Unknown preset: ${profileName}`);
 ...
 */
export function applyPreset(settings: Record<string, string | number | boolean>): void {
